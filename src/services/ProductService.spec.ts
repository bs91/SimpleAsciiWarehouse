...
describe('ProductService', () => {
    ...
    describe('getProducts', () => {
        ...
        let returnedPromiseNoParams: ng.IPromise<ng.IHttpPromiseCallbackArg<IProduct[]>>
        let returnedPromiseLimit: ng.IPromise<ng.IHttpPromiseCallbackArg<IProduct[]>>
        let returnedPromiseSkip: ng.IPromise<ng.IHttpPromiseCallbackArg<IProduct[]>>
        let returnedPromiseSort: ng.IPromise<ng.IHttpPromiseCallbackArg<IProduct[]>>
        let returnedPromiseLimitSort: ng.IPromise<ng.IHttpPromiseCallbackArg<IProduct[]>>
        let returnedPromiseLimitSkip: ng.IPromise<ng.IHttpPromiseCallbackArg<IProduct[]>>
        let returnedPromiseSkipSort: ng.IPromise<ng.IHttpPromiseCallbackArg<IProduct[]>>
        let returnedPromiseAllParams: ng.IPromise<ng.IHttpPromiseCallbackArg<IProduct[]>>

        beforeEach(() => {       
            $httpBackend.expect('GET', '/api/products?limit=10&skip=0&sort=id').respond(mockRespond);
            $httpBackend.expect('GET', '/api/products?limit=5&skip=0&sort=id').respond(mockRespond);
            $httpBackend.expect('GET', '/api/products?limit=10&skip=5&sort=id').respond(mockRespond);
            $httpBackend.expect('GET', '/api/products?limit=10&skip=0&sort=id').respond(mockRespond);
            $httpBackend.expect('GET', '/api/products?limit=5&skip=0&sort=id').respond(mockRespond);
            $httpBackend.expect('GET', '/api/products?limit=5&skip=5&sort=id').respond(mockRespond);
            $httpBackend.expect('GET', '/api/products?limit=10&skip=5&sort=id').respond(mockRespond);
            $httpBackend.expect('GET', '/api/products?limit=5&skip=5&sort=id').respond(mockRespond);

            returnedPromiseNoParams = localProductService.getProducts();
            returnedPromiseLimit = localProductService.getProducts(5);
            returnedPromiseSkip = localProductService.getProducts(undefined, 5);
            returnedPromiseSort = localProductService.getProducts(undefined, undefined, 'id');
            returnedPromiseLimitSort = localProductService.getProducts(5, undefined, 'id');
            returnedPromiseLimitSkip = localProductService.getProducts(5,5);
            returnedPromiseSkipSort = localProductService.getProducts(undefined, 5, 'id');
            returnedPromiseAllParams = localProductService.getProducts(5,5,'id');
        });

        it('should call all the apropriate urls with the given parameters', () => {
            expect($httpBackend.flush).not.toThrow();
        });

        it('should successfuly return data for all calls', () => {
            $httpBackend.flush();

            returnedPromiseNoParams.then((result: any) => {
                expect(result.data).toEqual(transformData(mockRespond));
            });

            returnedPromiseLimit.then((result: any) => {
                expect(result.data).toEqual(transformData(mockRespond));
            });

            returnedPromiseSkip.then((result: any) => {
                expect(result.data).toEqual(transformData(mockRespond));
            });

            returnedPromiseSort.then((result: any) => {
                expect(result.data).toEqual(transformData(mockRespond));
            });

            returnedPromiseLimitSort.then((result: any) => {
                expect(result.data).toEqual(transformData(mockRespond));
            });

            returnedPromiseLimitSkip.then((result: any) => {
                expect(result.data).toEqual(transformData(mockRespond));
            });

            returnedPromiseSkipSort.then((result: any) => {
                expect(result.data).toEqual(transformData(mockRespond));
            });

            returnedPromiseAllParams.then((result: any) => {
                expect(result.data).toEqual(transformData(mockRespond));
            });
